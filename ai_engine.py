...
RISK_TITLE: [общее название риска — БЕЗ номеров пунктов, БЕЗ юридических терминов, простыми словами]
RISK_CATEGORY: [одна фраза — к чему относится риск: деньги / сроки / ответственность / расторжение / конфиденциальность]

ПРАВИЛА — очень важно:
- НЕ указывай номера пунктов договора (не пиши "пункт 6.4" и т.п.)
- НЕ давай рекомендаций что делать
- НЕ объясняй подробно что происходит
- НЕ цитируй договор
- Название риска — общее, тревожное, без деталей. Пример: "Отсутствие защиты от финансовых потерь"
...
def format_pro_result(data: dict) -> list:
    ...
    final += (
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "⚖️ <i>Результат сформирован с использованием технологий искусственного "
        "интеллекта и носит информационно-рекомендательный характер. "
        "Не является юридическим заключением и не заменяет консультацию юриста.</i>\n\n"
